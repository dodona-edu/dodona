  ...
  def update
    respond_to do |format|
      if @series.update(permitted_attributes(@series))
        format.html { redirect_to course_path(@series.course, series: @series, anchor: "series-#{@series.id}"), notice: I18n.t('controllers.updated', model: Series.model_name.human) }
