...
class Activity < ApplicationRecord
  ...
  scope :order_by_name, ->(direction) { order("name_#{I18n.locale} #{direction}").order(path: direction) }
  scope :order_by_popularity, ->(direction) { joins(:courses).group(:id).order(Arel.sql("COUNT(DISTINCT courses.id) #{direction}")) }
