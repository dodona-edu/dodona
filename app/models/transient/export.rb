...
class Export
  ...
  def valid?
    @errors = []
    @errors.push(I18n.t('export.start_download.no_selection')) if @list.blank? # No choices supplied, nothing to download
    @errors.push(I18n.t('export.start_download.unknown_grouping_option')) unless ['user', 'exercise', 'personal', nil].include? @options[:group_by]
    @errors.empty?
  end
