...
class ReviewSessionControllerTest < ActionDispatch::IntegrationTest
  def setup
  ...
  test 'Can remove user from review' do
    post review_sessions_path, params: {
      review_session: {
        series_id: @series.id,
        deadline: DateTime.now + 4.days
    ...
    post remove_user_review_session_path(@series.review_session, user_id: @users.first.id, format: :js)
    assert_equal (@users.count - 1) * @exercises.count, @series.review_session.reviews.count
  end

  test 'Can add user to review' do
    post review_sessions_path, params: {
      review_session: {
        series_id: @series.id,
        deadline: DateTime.now + 4.days
    ...
    post add_user_review_session_path(@series.review_session, user_id: user.id, format: :js)
    assert_equal (@users.count + 1) * @exercises.count, @series.review_session.reviews.count
  end
