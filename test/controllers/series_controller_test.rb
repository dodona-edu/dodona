...
class SeriesControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should add activities to series' do
    ...
    assert_response :success
    assert_includes @instance.reload.activities, activity
  end
  ...
  test 'repository admin adding private activity to series should add course to repository\'s allowed courses' do
    ...
    assert_includes activity.repository.allowed_courses, @instance.course
  end
...
class SeriesVisibilityTest < ActionDispatch::IntegrationTest
  ...
  test 'student should only see visible series in course' do
    ...
    assert_equal 2, result_series.count, 'expected exactly 2 (visible, timed_now) series'

    assert_equal [@series.id, @timed_series_now.id].to_set, result_series.pluck(:id).to_set
  end
